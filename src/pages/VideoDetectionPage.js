...
const VideoDetectionPage = () => {
    ...
    const handleUpload = async () => {
        ...
        if (uploadMethod === 'file' && !selectedFile) {
            ...
            else if(!videoLink.startsWith("https://") &&!videoLink.startsWith("http://")) {
                warning('请输入正确的视频链接');
                return;
            }
        }
    ...
    const processVideoLink = async () => {
        ...
        try {
            const response = await axios.post(SERVER_VIDEO_LINK, { url: videoLink }, {
    ...
    const processResponse = (result) => {
        ...
        setIsHarmful(video_class !== 'normal' || audio_violation[0] !== '正常' || text_violation[0] !== '正常');
        let res = '';
        ...
        for (let i = 1; i < audio_violation.length; i++) {
            if (audio_violation[i] !== '') 
                audio_cat += '、'+audio_violation[i];
        }
        for (let i = 1; i < text_violation.length; i++) {
            if (text_violation[i] !== '')
                ocr_cat += '、' + text_violation[i];
        }
        if (tmp === 1) {
        ...
        setResults({
            summary: res,
            detections: [
                { id: 1, type: '视频类型', confidence: video_class_prob, description: video_class !== 'normal' ? class_en2zh[video_class] : '正常' },
                { id: 2, type: '次要类型', confidence: video_class_2_prob, description: video_class_2 !== 'normal' ? class_en2zh[video_class_2] : '正常' },
                { id: 3, type: '音频检测', confidence: audio_violation_prob, description: audio_violation[0] !== '正常' ? audio_violation[0] : '正常' },
                { id: 4, type: '文本检测', confidence: text_violation_prob, description: text_violation[0] !== '正常' ? text_violation[0] : '正常' }
            ]
        });
        success('视频检测完成!');
    };
                    ...
                    <div className="video-upload-panel">
                        ...
                        {uploadMethod === 'file' ? (
                                ...
                                {!videoPreview ? (
                                    <Dragger {...uploadProps}>
                                        <p className="ant-upload-drag-icon">
                                            <InboxOutlined style={{ color: '#0175C2', fontSize: 48 }} />
                                        </p>
                                    ...
                                    <div className="video-upload-preview-row" style={{ flexDirection: 'column', alignItems: 'center' }}>
                                        <div className="video-preview-box" style={{ display: 'flex', justifyContent: 'center' }}>
                                            <video
                                                controls
                                                src={videoPreview}
                            ...
                            <div className="link-input-container">
                                <p>输入bilibili、抖音等平台的视频链接，系统自动解析视频，给出视频是否违规以及违规种类</p>            
                                <Input
                                    style={{ width: '100%' }}
                                    placeholder="请输入视频链接..."
                                    value={videoLink}
                ...
                {isProcessing && (
                    <div className="processing-indicator">
                        ...
                        {/* <div style={{ width: 320, margin: '0 auto' }}>
                            <Progress percent={Math.round(detectionProgress * 100)} status="active" />
                        </div> */}
                    </div>
                )}
                                ...
                                {showViolationText && (
                                    ...
                                    >
                                        <div>
                                            ...
                                            {Array.isArray(audio_violation_text) && audio_violation_text.length > 0
                                        ...
                                        <div style={{ marginTop: 8 }}>
                                            ...
                                            {Array.isArray(ocr_violation_text) && ocr_violation_text.length > 0
